...
export function PublicBooking({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data: p } = await supabase
        .from("profiles")
        .select("id, full_name, avatar_url, bio, phone, active, allow_online, allow_presencial")
        .eq("slug", slug)
        .maybeSingle();
      if (!p || !p.active) {
        setLoading(false);
  ...
  const submit = async () => {
    ...
    // Rate limiting: máximo 5 agendamentos por hora por navegador
    const RATE_KEY = "seta:booking-timestamps";
    const now = Date.now();
    const stored = JSON.parse(localStorage.getItem(RATE_KEY) ?? "[]") as number[];
    const recent = stored.filter((t) => now - t < 3600000); // última hora
    if (recent.length >= 5) {
      toast.error("Muitos agendamentos em pouco tempo. Aguarde alguns minutos.");
      return;
    }
      ...
      <div className="flex min-h-screen flex-col bg-secondary">
        ...
        <div className="mx-auto w-full max-w-2xl flex-1 px-4 py-10 sm:py-14">
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)]">
            ...
            >
              <div className="confetti-burst mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-background/15 ring-4 ring-background/25 backdrop-blur">
  ...
  return (
    <div className="flex min-h-screen flex-col bg-secondary">
      ...
      {profile.phone && (
        <a
          href={`https://wa.me/${profile.phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Olá ${profile.full_name}, gostaria de tirar uma dúvida sobre o agendamento.`)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-transform hover:scale-110"
          aria-label="Conversar no WhatsApp"
          title={`Falar com ${profile.full_name} no WhatsApp`}
        >
