-- Corrige o trigger validate_appointment para permitir atualizações de status
-- em reuniões que já passaram (concluir, cancelar, remarcar).
-- A validação de "data passada" só deve bloquear INSERT de novos agendamentos,
-- não UPDATE de status em agendamentos existentes.
...
BEGIN
  ...
  -- Datas passadas: bloqueia apenas INSERT de novos agendamentos.
  -- Em UPDATE, permite mudanças de status (concluir, cancelar) em reuniões passadas.
  IF TG_OP = 'INSERT' AND (NEW.appointment_date + NEW.start_time) < now() THEN
    RAISE EXCEPTION 'Não é possível agendar em datas passadas';
  END IF;

  -- Em UPDATE: se só mudou status/notes/internal_notes/feedback, pula validações de slot
  IF TG_OP = 'UPDATE' THEN
    IF OLD.appointment_date = NEW.appointment_date
       ...
       AND OLD.meeting_type = NEW.meeting_type THEN
      -- Apenas mudança de status/notas — não precisa revalidar slot
      RETURN NEW;
    END IF;
    -- Se mudou data/horário para o passado, bloqueia
